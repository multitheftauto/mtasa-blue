...
{
    char data[24];
    bool operator<(const STagString& other) const { return strcmp(data, other.data) < 0; }
};
...
MTAEXPORT unsigned long GetAllocStats(uint uiType, void* pOutData, unsigned long ulNumStats)
{
    if (uiType == 0)
    {
        ...
        if (ulNumStats > 1)
            pOutStats[1] = pAllocStats->TotalMemMax;
        if (ulNumStats > 2)
            pOutStats[2] = pAllocStats->ActiveAllocs;
        if (ulNumStats > 3)
            pOutStats[3] = pAllocStats->DupeAllocs;
        if (ulNumStats > 4)
            pOutStats[4] = pAllocStats->UniqueAllocs;
        if (ulNumStats > 5)
            pOutStats[5] = pAllocStats->ReAllocs;
        if (ulNumStats > 6)
            pOutStats[6] = pAllocStats->Frees;
        if (ulNumStats > 7)
            pOutStats[7] = pAllocStats->UnmatchedFrees;
        if (ulNumStats > 8)
