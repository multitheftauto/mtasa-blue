...
void CPerfStatFunctionTimingImpl::GetStats(CPerfStatResult* pResult, const std::map<SString, int>& optionMap, const SString& strFilter)
{
    ...
    for (std::map<SString, SFunctionTimingInfo>::const_iterator iter = m_TimingMap.begin(); iter != m_TimingMap.end(); iter++)
    {
        ...
        {
            row[c++] = SString("{}", prev5s.uiNumCalls);

            row[c++] = SString("{:2.0f} ms", prev5s.fTotalMs);
            row[c++] = SString("{:2.0f} ms", prev5s.fPeakMs);
            row[c++] = SString("{:2.0f} ms ({})", prev5s.fResBiggestMs, *prev5s.strResBiggestMsName);

            row[c++] = prev5s.uiTotalBytes < 10 ? "" : SString("{}", *CPerfStatManager::GetScaledByteString(prev5s.uiTotalBytes));
            // row[c++] = prev5s.uiPeakBytes < 10 ? "" : SString ( "{} ", *CPerfStatManager::GetScaledByteString( prev5s.uiPeakBytes ) );
            row[c++] = prev5s.uiResBiggestBytes < 10
                           ? ""
                           : SString("{} ({})", *CPerfStatManager::GetScaledByteString(prev5s.uiResBiggestBytes), *prev5s.strResBiggestBytesName);
        }

        row[c++] = SString("{}", prev60s.uiNumCalls);

        row[c++] = SString("{:2.0f} ms", prev60s.fTotalMs);
        row[c++] = SString("{:2.0f} ms", prev60s.fPeakMs);
        row[c++] = SString("{:2.0f} ms ({})", prev60s.fResBiggestMs, *prev60s.strResBiggestMsName);

        row[c++] = prev60s.uiTotalBytes < 10 ? "" : SString("{} ", *CPerfStatManager::GetScaledByteString(prev60s.uiTotalBytes));
        // row[c++] = prev60s.uiPeakBytes < 10 ? "" : SString ( "{} ", *CPerfStatManager::GetScaledByteString( prev60s.uiPeakBytes ) );
        row[c++] = prev60s.uiResBiggestBytes < 10
                       ? ""
                       : SString("{} ({})", *CPerfStatManager::GetScaledByteString(prev60s.uiResBiggestBytes), *prev60s.strResBiggestBytesName);
    }
}
