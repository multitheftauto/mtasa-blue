...
bool CResourceStartPacket::Write(NetBitStreamInterface& BitStream) const
{
    ...
    {
        for (const auto& pFile : m_pResource->GetFiles())
        {
            if (pFile->GetType() != CResourceScriptItem::RESOURCE_FILE_TYPE_CLIENT_SCRIPT)
                continue;

            if (!static_cast<CResourceClientScriptItem*>(pFile)->IsNoClientCache())
                continue;
    ...
    for (const auto& pFile : m_pResource->GetFiles())
    {
        if (pFile->GetType() != CResourceScriptItem::RESOURCE_FILE_TYPE_CLIENT_CONFIG ||
            !m_pResource->IsClientConfigsOn()
        )
            continue;
        if (pFile->GetType() != CResourceScriptItem::RESOURCE_FILE_TYPE_CLIENT_SCRIPT || !m_pResource->IsClientScriptsOn() ||
            static_cast<CResourceClientScriptItem*>(pFile)->IsNoClientCache())
            continue;
        if (pFile->GetType() != CResourceScriptItem::RESOURCE_FILE_TYPE_CLIENT_FILE || !m_pResource->IsClientFilesOn())
            continue;
        ...
        // Write the map name
        const char* szFileName = pFile->GetWindowsName();
        ...
        BitStream.Write(static_cast<unsigned char>(pFile->GetType()));
        CChecksum checksum = pFile->GetLastChecksum();
        BitStream.Write(checksum.ulCRC);
        BitStream.Write((const char*)checksum.md5.data, sizeof(checksum.md5.data));
        BitStream.Write(pFile->GetApproxSize());
        if (pFile->GetType() == CResourceScriptItem::RESOURCE_FILE_TYPE_CLIENT_FILE)
        {
            auto pRCFItem = reinterpret_cast<CResourceClientFileItem*>(pFile);
    ...
    for (const auto& pExportedFunction : m_pResource->GetExportedFunctions())
    {
        // Check to see if the exported function is 'client'
        if (pExportedFunction.GetType() != CExportedFunction::EXPORTED_FUNCTION_TYPE_CLIENT)
            continue;
        ...
        const auto& strFunctionName = pExportedFunction.GetFunctionName();
