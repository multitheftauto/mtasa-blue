...
bool CResource::Load()
{
    ...
    if (bParsedSuccessfully)
    {
        ...
        if (pRoot)
        {
            ...
            CXMLNode* pNodeSettings = pRoot->GetChild("settings", 0);
            if (pNodeSettings)
            ...
            CXMLNode* pNodeMinMtaVersion = pRoot->GetChild("min_mta_version", 0);
            ...
            CXMLNode* pNodeAclRequest = pRoot->GetChild("aclrequest", 0);
            ...
            CXMLNode* pNodeSyncMapElementData = pRoot->GetChild("sync_map_element_data", 0);
                ...
                if (pRootNode)
                {
                    ...
                    else
                    {
                        ...
                        if (pAttr)
                            ...
                            pInfoNode->AddAttribute(szKey)->SetValue(szValue);
                    }
...
bool CResource::ReadIncludedHTML(CXMLNode* pRoot)
{
    ...
    for (CXMLNode* pHTML = pRoot->GetChild("html", i); pHTML != nullptr; pHTML = pRoot->GetChild("html", ++i))
    {
        // See if this is the default page (default attribute)
        bool           bIsDefault = false;
        CXMLAttribute* pDefaultfile = pHTML->GetAttribute("default");
        if (pDefaultfile)
        {
            auto strDefault = pDefaultfile->GetValue();
        ...
        bool           bIsRaw = false;
        CXMLAttribute* pRaw = pHTML->GetAttribute("raw");
        if (pRaw)
        {
            auto strRaw = pRaw->GetValue();
        ...
        CXMLAttribute* pRestricted = pHTML->GetAttribute("restricted");
        if (pRestricted)
        {
            auto strRestricted = pRestricted->GetValue();
        ...
        CXMLAttribute* pSrc = pHTML->GetAttribute("src");
        if (pSrc)
        {
            ...
            if (!strFilename.empty())
            {
                ...
                if (IsValidFilePath(strFilename.c_str()) && GetFilePath(strFilename.c_str(), strFullFilename))
                {
                    ...
                    auto pResourceFile = new CResourceHTMLItem(this, strFilename.c_str(), strFullFilename.c_str(), pHTML->GetAttributes(), bIsDefault, bIsRaw,
...
bool CResource::ReadIncludedConfigs(CXMLNode* pRoot)
{
    ...
    for (CXMLNode* pConfig = pRoot->GetChild("config", i); pConfig != nullptr; pConfig = pRoot->GetChild("config", ++i))
    {
        bool           bServer = true;
        bool           bClient = false;
        CXMLAttribute* pType = pConfig->GetAttribute("type");
        if (pType)
        {
            // Grab the type. Client or server or shared
            std::string strType = pType->GetValue();
            if (!strType.empty())
            {
                ...
                if (!stricmp(szType, "client"))
                {
                    ...
                    CLogger::LogPrintf("Unknown config type specified in %s. Got '%s'. Assuming 'server'\n", m_strResourceName.c_str(), szType);
            }
        }

        // Find the source (file path)
        CXMLAttribute* pSrc = pConfig->GetAttribute("src");
        if (pSrc)
        {
            // Grab the filename
            std::string strFilename = pSrc->GetValue();
            ...
            if (!strFilename.empty())
            {
                ...
                if (IsValidFilePath(strFilename.c_str()) && GetFilePath(strFilename.c_str(), strFullFilename))
                {
                    ...
                    if (bServer)
                        m_ResourceFiles.push_back(new CResourceConfigItem(this, strFilename.c_str(), strFullFilename.c_str(), pConfig->GetAttributes()));

                    if (bClient)
                        m_ResourceFiles.push_back(new CResourceClientConfigItem(this, strFilename.c_str(), strFullFilename.c_str(), pConfig->GetAttributes()));
                }
                else
                {
...
bool CResource::ReadIncludedFiles(CXMLNode* pRoot)
{
    ...
    for (CXMLNode* pFile = pRoot->GetChild("file", i); pFile != nullptr; pFile = pRoot->GetChild("file", ++i))
    {
        // Grab the filepath attribute
        CXMLAttribute* pSrc = pFile->GetAttribute("src");
        ...
        if (pSrc)
        {
            ...
            if (!strFilename.empty())
            {
                ...
                bool           bDownload = true;
                CXMLAttribute* pDownload = pFile->GetAttribute("download");
                if (pDownload)
                {
                    auto strDownload = pDownload->GetValue();
                ...
                if (IsValidFilePath(strFilename.c_str()) && GetFilePath(strFilename.c_str(), strFullFilename))
                {
                    m_ResourceFiles.push_back(new CResourceClientFileItem(this, strFilename.c_str(), strFullFilename.c_str(), pFile->GetAttributes(), bDownload));
                }
                else
                {
...
bool CResource::ReadIncludedExports(CXMLNode* pRoot)
{
    ...
    // Read our exportlist
    for (CXMLNode* pExport = pRoot->GetChild("export", i); pExport != nullptr; pExport = pRoot->GetChild("export", ++i))
    {
        // See if the http attribute is true or false
        bool           bHTTP = false;
        CXMLAttribute* pHttp = pExport->GetAttribute("http");
        if (pHttp)
        {
            auto strHttp = pHttp->GetValue();
        ...
        CXMLAttribute* pRestricted = pExport->GetAttribute("restricted");
        if (pRestricted)
        {
            auto strRestricted = pRestricted->GetValue();
            if (strRestricted == "yes" || strRestricted == "true")
        ...
        CXMLAttribute* pType = pExport->GetAttribute("type");
        if (pType)
        {
            // Grab the type. Client or server or shared
            std::string strType = pType->GetValue();
            if (!strType.empty())
            {
                ...
                if (!stricmp(szType, "client"))
                {
                    ...
                    CLogger::LogPrintf("Unknown exported function type specified in %s. Got '%s'. Assuming 'server'\n", m_strResourceName.c_str(), szType);
            }
        }

        // Grab the functionname attribute
        CXMLAttribute* pFunction = pExport->GetAttribute("function");
        if (pFunction)
        {
...
bool CResource::ReadIncludedScripts(CXMLNode* pRoot)
{
    ...
    for (CXMLNode* pScript = pRoot->GetChild("script", i); pScript != nullptr; pScript = pRoot->GetChild("script", ++i))
    {
        // Grab the type attribute (server / client)
        bool           bServer = true;
        bool           bClient = false;
        CXMLAttribute* pType = pScript->GetAttribute("type");
        if (pType)
        {
            // Grab the type. Client or server or shared
            std::string strType = pType->GetValue();
            if (!strType.empty())
            {
                ...
                if (!stricmp(szType, "client"))
                {
                    ...
                    CLogger::LogPrintf("Unknown script type specified in %s. Got '%s'. Assuming 'server'\n", m_strResourceName.c_str(), szType);
            }
        }

        // Grab the source attribute
        CXMLAttribute* pSrc = pScript->GetAttribute("src");
        if (pSrc)
        {
            ...
            if (!strFilename.empty())
            {
                ...
                if (IsValidFilePath(strFilename.c_str()) && GetFilePath(strFilename.c_str(), strFullFilename))
                {
                    ...
                    if (bServer)
                        m_ResourceFiles.push_back(new CResourceScriptItem(this, strFilename.c_str(), strFullFilename.c_str(), pScript->GetAttributes()));
                    if (bClient)
                        m_ResourceFiles.push_back(new CResourceClientScriptItem(this, strFilename.c_str(), strFullFilename.c_str(), pScript->GetAttributes()));
                }
                else
                {
...
bool CResource::ReadIncludedMaps(CXMLNode* pRoot)
{
    ...
    for (CXMLNode* pMap = pRoot->GetChild("map", i); pMap != nullptr; pMap = pRoot->GetChild("map", ++i))
    {
        // Grab the dimension attribute
        int            iDimension = 0;
        CXMLAttribute* pDimension = pMap->GetAttribute("dimension");
        if (pDimension)
        {
            iDimension = atoi(std::string(pDimension->GetValue()).c_str());
            if (iDimension < 0 || iDimension > 65535)
                iDimension = 0;
        }

        // Grab the source node
        CXMLAttribute* pSrc = pMap->GetAttribute("src");
        if (pSrc)
        {
            std::string strFilename = pSrc->GetValue();
            ...
            if (!strFilename.empty())
            {
                ...
                if (IsValidFilePath(strFilename.c_str()) && GetFilePath(strFilename.c_str(), strFullFilename))
                {
                    m_ResourceFiles.push_back(new CResourceMapItem(this, strFilename.c_str(), strFullFilename.c_str(), pMap->GetAttributes(), iDimension));
                }
                else
                {
...
bool CResource::GetDefaultSetting(const char* szName, char* szValue, size_t sizeBuffer)
{
    ...
    for (auto& pTemp : pRoot->GetChildren())
    {
        ...
        if (!stricmp(strTagName.c_str(), "setting"))
        {
            ...
            CXMLAttribute* pName = pTemp->GetAttribute("name");
...
bool CResource::ReadIncludedResources(CXMLNode* pRoot)
{
    ...
    for (CXMLNode* pInclude = pRoot->GetChild("include", i); pInclude != nullptr; pInclude = pRoot->GetChild("include", ++i))
    {
        // Grab the minversion attribute (minimum version the included resource needs to be)
        SVersion       svMinVersion;
        CXMLAttribute* pMinVersion = pInclude->GetAttribute("minversion");
        if (pMinVersion)
        {
        ...
        CXMLAttribute* pMaxVersion = pInclude->GetAttribute("maxversion");
        if (pMaxVersion)
        {
        ...
        CXMLAttribute* pSrc = pInclude->GetAttribute("resource");
        if (pSrc)
        {
