...
bool CVehicle::ReadSpecialData(const int iLine)
{
    ...
    // Grab the "forceColor" data
    // See GitHub #263: If you provide 0,0,0 as the "color" attribute value, will set color to random
    bool bForceColor = false;
    GetCustomDataBool("forceColor", bForceColor, true);
    ...
    if (GetCustomDataString("color", szTemp, 256, true))
    {
        ...
        if (i == 3 || i == 6 || i == 9 || i == 12)
        {
            m_Color.SetRGBColors(SColorRGBA(ucValues[0], ucValues[1], ucValues[2], 0), SColorRGBA(ucValues[3], ucValues[4], ucValues[5], 0),
                                 SColorRGBA(ucValues[6], ucValues[7], ucValues[8], 0), SColorRGBA(ucValues[9], ucValues[10], ucValues[11], 0), bForceColor);
        }
        else
        {
            m_Color.SetPaletteColors(ucValues[0], ucValues[1], ucValues[2], ucValues[3], bForceColor);
        }
    }
