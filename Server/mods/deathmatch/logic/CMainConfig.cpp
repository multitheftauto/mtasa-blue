...
bool CMainConfig::AddMissingSettings()
{
    ...
    for (auto& pNodeTemplate : pRootNodeTemplate->GetChildren())
    {
        SString   strNodeName = pNodeTemplate->GetTagName();
        CXMLNode* pNode = m_pRootNode->GetChild(strNodeName);
        if (!pNode)
        {
            ...
            pNode = m_pRootNode->CreateChild(strNodeName, pPrevNode);
