...
bool CClientModelRequestManager::IsLoaded(std::uint16_t usModelID)
{
    // Grab the model info
...
bool CClientModelRequestManager::RequestBlocking(std::uint16_t usModelID, const char* szTag)
{
    // Grab the model info
    CModelInfo* pInfo = g_pGame->GetModelInfo(usModelID);
    if (pInfo)
...
bool CClientModelRequestManager::Request(std::uint16_t usModelID, CClientEntity* pRequester)
{
    assert(pRequester);
