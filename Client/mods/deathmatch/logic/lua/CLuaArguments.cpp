...
void CLuaArguments::CopyRecursive(const CLuaArguments& Arguments, CFastHashMap<CLuaArguments*, CLuaArguments*>* pKnownTables)
{
    ...
    {
        CLuaArgument* pArgument = new CLuaArgument(**iter, pKnownTables, this);
...
void CLuaArguments::ReadArguments(lua_State* luaVM, signed int uiIndexBegin)
{
    ...
    while (lua_type(luaVM, uiIndexBegin) != LUA_TNONE)
    {
        ...
        CLuaArgument* pArgument = new CLuaArgument(luaVM, uiIndexBegin++, &knownTables, this);
        m_Arguments.push_back(pArgument);
...
void CLuaArguments::ReadTable(lua_State* luaVM, int iIndexBegin, CFastHashMap<const void*, CLuaArguments*>* pKnownTables)
{
    ...
    while (lua_next(luaVM, iIndexBegin) != 0)
    {
        ...
        CLuaArgument* pArgument = new CLuaArgument(luaVM, -2, pKnownTables, this);
        m_Arguments.push_back(pArgument);            // push the key first

        pArgument = new CLuaArgument(luaVM, -1, pKnownTables, this);
...
{
    CLuaArgument* pArgument = new CLuaArgument(luaVM, iIndex, {}, this);
    m_Arguments.push_back(pArgument);
}
...
void CLuaArguments::PushArguments(const CLuaArguments& Arguments)
{
    ...
    {
        CLuaArgument* pArgument = new CLuaArgument(**iter, {}, this);
        m_Arguments.push_back(pArgument);
    }
}
...
{
    CLuaArgument* pArgument = new CLuaArgument(this);
    m_Arguments.push_back(pArgument);
    return pArgument;
}
...
{
    CLuaArgument* pArgument = new CLuaArgument(Argument, {}, this);
    m_Arguments.push_back(pArgument);
    return pArgument;
}
...
{
    CLuaArgument* pArgument = new CLuaArgument(std::move(argument), {}, this);
    m_Arguments.push_back(pArgument);
    return pArgument;
}

void CLuaArguments::PushArgumentWeak(const CLuaArgument* pArgument)
{
    m_Arguments.push_back(const_cast<CLuaArgument*>(pArgument));
}
...
void CLuaArguments::Pop()
{
    ...
    auto item = m_Arguments.back();
    if (item->m_pOwner == this)
        delete item;
...
bool CLuaArguments::ReadFromBitStream(NetBitStreamInterface& bitStream, std::vector<CLuaArguments*>* pKnownTables)
{
    ...
    if (bitStream.ReadCompressed(uiNumArgs))
    {
        ...
        {
            CLuaArgument* pArgument = new CLuaArgument(bitStream, pKnownTables, this);
            m_Arguments.push_back(pArgument);
        }
    }
    ...
    if (object)
    {
        if (json_object_get_type(object) == json_type_array)
        {
            ...
            for (uint i = 0; i < json_object_array_length(object); i++)
            {
                ...
                pArgument = new CLuaArgument(this);
