...
bool CClientIMG::LinkModel(unsigned int uiModel, size_t uiFileID)
{
    if (!IsStreamed())
        return false;
    ...
    CStreamingInfo* pCurrInfo = g_pGame->GetStreaming()->GetStreamingInfo(uiModel);

    if (pCurrInfo->archiveId == m_ucArchiveID)
        return true;            // Already linked

    m_restoreInfo.emplace_back(uiModel, pCurrInfo->offsetInBlocks, pCurrInfo->sizeInBlocks, pCurrInfo->archiveId);

    g_pGame->GetStreaming()->SetStreamingInfo(uiModel, m_ucArchiveID, pFileInfo->uiOffset, pFileInfo->usSize);

    return true;
}

bool CClientIMG::UnlinkModel(unsigned int uiModel)
{
    const auto it = std::find_if(m_restoreInfo.begin(), m_restoreInfo.end(), [uiModel](const auto& restoreInfo) { return restoreInfo.uiModel == uiModel; });

    if (it == m_restoreInfo.end())
        return false;

    g_pGame->GetStreaming()->SetStreamingInfo(uiModel, it->ucStreamID, it->uiOffset, it->usSize);
