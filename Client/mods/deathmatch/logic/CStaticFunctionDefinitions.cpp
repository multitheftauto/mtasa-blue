...
int CStaticFunctionDefinitions::GetElementCount(EElementCountType eElementType)
{
    switch (eElementType)
    {
        ...
        case ELEMENT_PED:
            return g_pClientGame->GetPedManager()->Count() - m_pPlayerManager->Count();
        case ELEMENT_WATER:
            return g_pClientGame->GetManager()->GetWaterManager()->Count();
        ...
        case ELEMENT_COLSHAPE:
        {
            ...
            for (; iter != m_pColManager->IterEnd(); iter++)
                if (!(*iter)->HasHitCallback())            // filter away markers and pickups
                    count++;

            return count;
        }
        case ELEMENT_BLIP:
            return g_pClientGame->GetManager()->GetRadarMarkerManager()->Count();
        case ELEMENT_RADARAREA:
            return g_pClientGame->GetManager()->GetRadarAreaManager()->Count();
        case ELEMENT_TEAM:
            return g_pClientGame->GetTeamManager()->Count();
    }
    return -1;
}
