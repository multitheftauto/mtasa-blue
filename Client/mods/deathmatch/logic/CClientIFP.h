...
public:
    ...
    std::bitset<64>                 GetAnimatedBonesMask(const SString& strAnimationName);
...
private:
    ...
    WORD         ReadSequencesWithDummies(std::unique_ptr<CAnimBlendHierarchy>& pAnimationHierarchy, std::bitset<64>& outAnimatedBonesMask);
    ...
    void  MoveSequencesWithDummies(std::unique_ptr<CAnimBlendHierarchy>&                 pAnimationHierarchy,
                                   std::map<DWORD, std::unique_ptr<CAnimBlendSequence>>& mapOfSequences, std::bitset<64>& outAnimatedBonesMask);
