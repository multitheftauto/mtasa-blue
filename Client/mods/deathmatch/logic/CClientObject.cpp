...
void CClientObject::SetVisibleInAllInteriors(bool bVisible, unsigned char ucNewInterior)
{
    m_bVisibleInAllInteriors = bVisible;

    // Stream-in/out the object as needed
    this->SetInterior(bVisible ? g_pClientGame->GetLocalPlayer()->GetInterior() : ucNewInterior);
}
