...
CLocalServer::~CLocalServer()
{
    if (m_pConfig)
        delete m_pConfig;
    //delete m_pWindow; /* TODO AFTER CEGUI API REWRITE */
}
...
bool CLocalServer::Save()
{
    if (m_pConfig && m_pConfig->GetRootNode())
    {
        ...
        StoreConfigValue("servername", "MTA Local Server");
        StoreConfigValue("maxplayers", "32");
        StoreConfigValue("donotbroadcastlan", "1");
        StoreConfigValue("ase", "0");
        StoreConfigValue("password", "");
