...
SFixedArray<std::vector<SPlayerClothing>, PLAYER_CLOTHING_SLOTS> CClientPlayerClothes::m_DefaultClothes{
    // Torso Clothing
    std::vector<SPlayerClothing>{{"player_torso", "torso"},
                                 {"vestblack", "vest"},
                                 {"vest", "vest"},
    ...
    std::vector<SPlayerClothing>{
        {"player_face", "head"},   {"hairblond", "head"},     {"hairred", "head"},       {"hairblue", "head"},       {"hairgreen", "head"},
        {"hairpink", "head"},      {"bald", "head"},          {"baldbeard", "head"},     {"baldtash", "head"},       {"baldgoatee", "head"},
        {"highfade", "head"},      {"highafro", "highafro"},  {"wedge", "wedge"},        {"slope", "slope"},         {"jhericurl", "jheri"},
        {"cornrows", "cornrows"},  {"cornrowsb", "cornrows"}, {"tramline", "tramline"},  {"groovecut", "groovecut"}, {"mohawk", "mohawk"},
        {"mohawkblond", "mohawk"}, {"mohawkpink", "mohawk"},  {"mohawkbeard", "mohawk"}, {"afro", "afro"},           {"afrotash", "afro"},
        {"afrobeard", "afro"},     {"afroblond", "afro"},     {"flattop", "flattop"},    {"elvishair", "elvishair"}, {"beard", "head"},
        {"tash", "head"},          {"goatee", "head"},        {"afrogoatee", "afro"}},

    // Legs Clothing
    std::vector<SPlayerClothing>{
        {"player_legs", "legs"},         {"worktrcamogrn", "worktr"},    {"worktrcamogry", "worktr"},   {"worktrgrey", "worktr"},   {"worktrkhaki", "worktr"},
        {"tracktr", "tracktr"},          {"tracktreris", "tracktr"},     {"jeansdenim", "jeans"},       {"legsblack", "legs"},      {"legsheart", "legs"},
        {"biegetr", "chinosb"},          {"tracktrpro", "tracktr"},      {"tracktrwhstr", "tracktr"},   {"tracktrblue", "tracktr"}, {"tracktrgang", "tracktr"},
        {"bbshortwht", "boxingshort"},   {"boxshort", "boxingshort"},    {"bbshortred", "boxingshort"}, {"shellsuittr", "tracktr"}, {"shortsgrey", "shorts"},
        {"shortskhaki", "shorts"},       {"chongergrey", "chonger"},     {"chongergang", "chonger"},    {"chongerred", "chonger"},  {"chongerblue", "chonger"},
        {"shortsgang", "shorts"},        {"denimsgang", "jeans"},        {"denimsred", "jeans"},        {"chinosbiege", "chinosb"}, {"chinoskhaki", "chinosb"},
        {"cutoffchinos", "shorts"},      {"cutoffchinosblue", "shorts"}, {"chinosblack", "chinosb"},    {"chinosblue", "chinosb"},  {"leathertr", "leathertr"},
        {"leathertrchaps", "leathertr"}, {"suit1trgrey", "suit1tr"},     {"suit1trblk", "suit1tr"},     {"cutoffdenims", "shorts"}, {"suit1trred", "suit1tr"},
        {"suit1trblue", "suit1tr"},      {"suit1tryellow", "suit1tr"},   {"suit1trgreen", "suit1tr"},   {"suit1trblk2", "suit1tr"}, {"suit1trgang", "suit1tr"}},

    // Shoes Clothing
    std::vector<SPlayerClothing>{{"foot", "feet"},
                                 {"cowboyboot2", "biker"},
                                 {"bask2semi", "bask1"},
                                 {"bask1eris", "bask1"},
    ...
    std::vector<SPlayerClothing>{{"4weed", "4WEED"}, {"4rip", "4RIP"}, {"4spider", "4SPIDER"}},

    // Left Lower Arm Clothing
    std::vector<SPlayerClothing>{{"5gun", "5GUN"}, {"5cross", "5CROSS"}, {"5cross2", "5CROSS2"}, {"5cross3", "5CROSS3"}},

    // Right Upper Arm Clothing
    std::vector<SPlayerClothing>{{"6aztec", "6AZTEC"}, {"6crown", "6CROWN"}, {"6clown", "6CLOWN"}, {"6africa", "6AFRICA"}},

    // Right LowerA rm Clothing
    std::vector<SPlayerClothing>{{"7cross", "7CROSS"}, {"7cross2", "7CROSS2"}, {"7cross3", "7CROSS3"}, {"7mary", "7MARY"}},

    // Back Top Clothing
    std::vector<SPlayerClothing>{
        {"8sa", "8SA"}, {"8sa2", "8SA2"}, {"8sa3", "8SA3"}, {"8westside", "8WESTSD"}, {"8santos", "8SANTOS"}, {"8poker", "8POKER"}, {"8gun", "8GUN"}},

    // Left Chest Clothing
    std::vector<SPlayerClothing>{
        {"9crown", "9CROWN"}, {"9gun", "9GUN"}, {"9gun2", "9GUN2"}, {"9homeboy", "9HOMBY"}, {"9bullet", "9BULLT"}, {"9rasta", "9RASTA"}},

    // Right Chest Clothing
    std::vector<SPlayerClothing>{
        {"10ls", "10LS"}, {"10ls2", "10LS2"}, {"10ls3", "10LS3"}, {"10ls4", "10LS4"}, {"10ls5", "10LS5"}, {"10og", "10OG"}, {"10weed", "10WEED"}},

    // Stomach Clothing
    std::vector<SPlayerClothing>{{"11grove", "11GROVE"},
                                 {"11grove2", "11GROV2"},
                                 {"11grove3", "11GROV3"},
                                 {"11dice", "11DICE"},
                                 {"11dice2", "11DICE2"},
    ...
    // Extra1 Clothing
    std::vector<SPlayerClothing>{{"dogtag", "neck"},
                                 {"neckafrica", "neck"},
                                 {"stopwatch", "neck"},
                                 {"necksaints", "neck"},
                                 {"neckhash", "neck"},
    ...
    // Extra2 Clothing
    std::vector<SPlayerClothing>{{"watchpink", "watch"},
                                 {"watchyellow", "watch"},
                                 {"watchpro", "watch"},
                                 {"watchpro2", "watch"},
                                 {"watchsub1", "watch"},
    ...
    // Extra3 Clothing
    std::vector<SPlayerClothing>{{"groucho", "grouchos"},
                                 {"zorro", "zorromask"},
                                 {"eyepatch", "eyepatch"},
                                 {"glasses01", "glasses01"},
                                 {"glasses04", "glasses04"},
    ...
    // Extra4 Clothing
    std::vector<SPlayerClothing>{{"bandred", "bandana"},
                                 {"bandblue", "bandana"},
                                 {"bandgang", "bandana"},
                                 {"bandblack", "bandana"},
                                 {"bandred2", "bandknots"},
    ...
    // Suit Clothing
    std::vector<SPlayerClothing>{{"gimpleg", "gimpleg"},
                                 {"valet", "valet"},
                                 {"countrytr", "countrytr"},
                                 {"croupier", "valet"},
                                 {"policetr", "policetr"},
