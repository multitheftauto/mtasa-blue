...
const SFixedArray<uchar, 212> g_ucMaxPassengers = {3, 1,   1,   1,   3,   3, 0,   1,  1, 3, 1, 1,   1, 3, 1, 1,            // 400->415
...
static const SFixedArray<ushort, 212> g_ulVehicleAttributes = {
...
bool CClientVehicleManager::IsTrainModel(ushort usModel)
{
    return (usModel == 449 || usModel == 537 || usModel == 538 || usModel == 569 || usModel == 590 || usModel == 570);
}

bool CClientVehicleManager::IsValidModel(ushort usModel)
{
    CModelInfo* pModelInfo = g_pGame->GetModelInfo(usModel);
    return pModelInfo && pModelInfo->IsVehicle();
}

bool CClientVehicleManager::IsStandardModel(ushort usModel)
{
    return usModel >= 400 && usModel <= 611;
}

eClientVehicleType CClientVehicleManager::GetVehicleType(ushort usModel)
{
    // Valid vehicle id?
...
uchar CClientVehicleManager::GetMaxPassenger(ushort usModel)
{
...
void CClientVehicleManager::GetRandomVariation(ushort usModel, uchar& ucVariant, uchar& ucVariant2)
{
    RandomizeRandomSeed();
    ucVariant = 255;
    ucVariant2 = 255;
...
uchar CClientVehicleManager::ConvertIndexToGameSeat(ushort usModel, uchar ucIndex)
{
    eClientVehicleType vehicleType = GetVehicleType(usModel);

    // Grab the max passenger count for the given ID
    uchar ucMaxPassengerCount = GetMaxPassenger(usModel);
...
bool CClientVehicleManager::HasDoors(ushort usModel)
{
...
void CClientVehicleManager::RestreamVehicles(ushort usModel)
{
...
void CClientVehicleManager::RestreamVehicleUpgrades(ushort ulModel)
{
    for (uint i = 0; i < m_List.size(); i++)
    {
        CClientVehicle* pVehicle = m_List[i];
