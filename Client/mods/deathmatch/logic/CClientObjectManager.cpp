...
static const std::uint16_t g_usBreakableModelList[] = {
...
bool CClientObjectManager::IsBreakableModel(std::uint32_t ulObjectModel)
{
    ...
    return breakableModelMap.find((std::uint16_t)ulObjectModel) != breakableModelMap.end();
}
...
bool CClientObjectManager::ObjectsAroundPointLoaded(const CVector& vecPosition, float fRadius, std::uint16_t usDimension, SString* pstrStatus)
{
...
void CClientObjectManager::UpdateLimitInfo()
{
    ...
    std::uint32_t nEntryInfoNodeEntries = pPools->GetEntryInfoNodePool()->GetNumberOfUsedSpaces();
    std::uint32_t nPointerNodeSingleLinkEntries = pPools->GetPointerNodeSingleLinkPool()->GetNumberOfUsedSpaces();
    std::uint32_t nPointerNodeDoubleLinkEntries = pPools->GetPointerNodeDoubleLinkPool()->GetNumberOfUsedSpaces();
...
void CClientObjectManager::RestreamObjects(std::uint16_t usModel)
{
