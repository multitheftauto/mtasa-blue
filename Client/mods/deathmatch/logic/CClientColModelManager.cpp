...
CClientColModel* CClientColModelManager::GetElementThatReplaced(std::uint16_t usModel, CClientColModel* pDontSearch)
{
    // Item exists in list?
    std::list<CClientColModel*>::iterator iter = m_List.begin();
    for (; iter != m_List.end(); iter++)
...
bool CClientColModelManager::IsReplacableModel(std::uint16_t usModel)
{
    // We can only replace object collisions atm
    return CClientObjectManager::IsValidModel(usModel);
}

bool CClientColModelManager::RestoreModel(std::uint16_t usModel)
{
