...
CClientDFF* CClientDFFManager::GetElementThatReplaced(std::uint16_t usModel, CClientDFF* pDontSearch)
{
    // Matches given DFF?
    std::list<CClientDFF*>::iterator iter = m_List.begin();
    for (; iter != m_List.end(); iter++)
...
bool CClientDFFManager::IsReplacableModel(std::uint16_t usModel)
{
...
bool CClientDFFManager::RestoreModel(std::uint16_t usModel)
{
