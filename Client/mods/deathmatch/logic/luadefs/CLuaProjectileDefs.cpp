        ...
        if (pLuaMain)
        {
            ...
            if (pResource)
            {
                ...
                if (pProjectile)
                {
                    ...
                    lua_pushobject(luaVM, pProjectile);
                    return 1;
                }
            }
        }
...
int CLuaProjectileDefs::GetProjectileTarget(lua_State* luaVM)
{
    ...
    if (!argStream.HasErrors())
    {
        ...
        {
            lua_pushobject(luaVM, pProjectile->GetTargetEntity());
            return 1;
        }
        else if (ucWeapon == WEAPONTYPE_REMOTE_SATCHEL_CHARGE)
        {
            lua_pushobject(luaVM, pProjectile->GetSatchelAttachedTo());
            return 1;
        }
    }
    else
...
int CLuaProjectileDefs::GetProjectileCreator(lua_State* luaVM)
{
    ...
    if (!argStream.HasErrors())
    {
        lua_pushobject(luaVM, pProjectile->GetCreator());
        return 1;
    }
    else
        m_pScriptDebugging->LogCustom(luaVM, argStream.GetFullErrorMessage());
