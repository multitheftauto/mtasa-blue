                                    ...
                                    CClientPlayer* pBackupDamagedPlayer)
{
    if (m_pPlayerPed)
    {
        ...
        if (weaponType != GetCurrentWeaponType())
        {
            ...
            AddReportLog(5432, SString("DischargeWeapon adding missing weapon {} ({})", weaponType, GetNick()), 30);
        }
        ...
        if (pBackupDamagedPlayer && pBackupDamagedPlayer->GetGamePlayer() && pBackupDamagedPlayer->GetWasRecentlyInNetworkInterruption(1000))
        {
            ...
            if (fBackupDamage > 0)
            {
                ...
                SString strMessage("Applied {:0.2f} damage to {} (from {}) due to network interruption", fBackupDamage, pBackupDamagedPlayer->GetNick(),
                                   GetNick());
