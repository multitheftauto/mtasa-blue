...
public:
    ...
    virtual void         SetType(const eWeaponType& type) = 0;

    virtual eWeaponState GetState() const = 0;
    virtual void         SetState(const eWeaponState& state) = 0;
    ...
    virtual CWeaponInfo* GetInfo(const eWeaponSkill& skill) const = 0;
    ...
    virtual bool GenerateDamageEvent(CPed* ped, CEntity* responsible, eWeaponType weaponType, int damagePerHit, ePedPieceTypes hitZone, std::uint8_t dir) const = 0;

    virtual bool ProcessLineOfSight(const CVector* vecStart, const CVector* vecEnd, CColPoint** colCollision, CEntity** collisionEntity, const SLineOfSightFlags& flags, SLineOfSightBuildingResult* buildingResult, const eWeaponType& weaponType, CEntitySAInterface** entity) = 0;
