...
CGUIGridList_Impl::CGUIGridList_Impl(CGUI_Impl* pGUI, CGUIElement* pParent, bool bFrame)
{
    ...
    if (bFrame)
        m_pWindow = pGUI->GetWindowManager()->createWindow(pGUI->GetElementPrefix() + "/" + CGUIGRIDLIST_NAME, szUnique);
    else
        m_pWindow = pGUI->GetWindowManager()->createWindow(pGUI->GetElementPrefix() + "/" + CGUIGRIDLISTNOFRAME_NAME, szUnique);
...
unsigned int CGUIGridList_Impl::AddColumn(const char* szTitle, float fWidth)
{
    ...
    reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->addColumn((CGUI_Impl::GetUTFString(szTitle)), hUniqueHandle, CEGUI::UDim(0, fWidth));
...
void CGUIGridList_Impl::SetColumnWidth(int hColumn, float fWidth, bool bRelative)
{
    ...
    if (bRelative)
        width = CEGUI::UDim(fWidth, 0);
    else
        width = CEGUI::UDim(0, fWidth);

    reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->setColumnHeaderWidth(GetColumnIndex(hColumn), width);
}

bool CGUIGridList_Impl::GetColumnWidth(int hColumn, float& fOutWidth, bool bRelative)
{
    CEGUI::UDim uWidth = reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->getColumnHeaderWidth(GetColumnIndex(hColumn));
    fOutWidth = bRelative ? uWidth.d_scale : uWidth.d_scale;
    return true;
}

void CGUIGridList_Impl::SetColumnTitle(int hColumn, const char* szTitle)
{
    //reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow)->(GetColumnIndex(hColumn), szTitle);
}
...
int CGUIGridList_Impl::SetRowItemsText(int iRow, std::vector<std::pair<SString, bool> >* m_items)
{
    ...
    for (it; it != m_items->end(); it++)
    {
        ...
        if (col != iSortColumn)
            SetItemText(iRow, col, it->first.c_str(), it->second);
        col++;
    }
    // Add our sort column text, if it was provided
    if ((uint)iSortColumn <= m_items->size())
        iRow = SetItemText(iRow, iSortColumn, m_items->at(iSortColumn - 1).first.c_str(), m_items->at(iSortColumn - 1).second);
...
void CGUIGridList_Impl::SetColumnSegmentSizingEnabled(int hColumn, bool bEnabled)
{
    try
    {
        CEGUI::MultiColumnList* columnList = reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow);
        CEGUI::ListHeaderSegment* columnSegment = &columnList->getHeaderSegmentForColumn(hColumn);
        columnSegment->setSizingEnabled(bEnabled);
    }
    catch (CEGUI::Exception)
    {
    }
...
bool CGUIGridList_Impl::IsColumnSegmentSizingEnabled(int hColumn)
{
    try
    {
        CEGUI::MultiColumnList* columnList = reinterpret_cast<CEGUI::MultiColumnList*>(m_pWindow);
        CEGUI::ListHeaderSegment* columnSegment = &columnList->getHeaderSegmentForColumn(hColumn);
        return columnSegment->isSizingEnabled();
    }
    catch (CEGUI::Exception)
    {
        return false;
