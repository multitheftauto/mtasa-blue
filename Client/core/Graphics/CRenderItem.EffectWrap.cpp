...
#define USING_LIST "RenderState"
    ...
    ADD_REGISTER(TYPE_D3DCOLOR, BLENDFACTOR)            //  = 193,   /* D3DCOLOR used for a constant blend factor during alpha blending for devices that support
    ...
    ADD_REGISTER(TYPE_INT,
                 SRCBLENDALPHA)            //  = 207,  /* SRC blend factor for the alpha channel when DWORD SEPARATEDESTALPHAENABLE )        //is TRUE */
    ADD_REGISTER(TYPE_INT,
                 DESTBLENDALPHA)            //  = 208,  /* DST blend factor for the alpha channel when DWORD SEPARATEDESTALPHAENABLE )        //is TRUE */
    ADD_REGISTER(TYPE_INT,
                 BLENDOPALPHA)            //  = 209,  /* Blending operation for the alpha channel when DWORD SEPARATEDESTALPHAENABLE )        //is TRUE */
#undef  USING_STRUCT
#undef  USING_LIST
...
#define USING_LIST "SamplerState"
    ...
    ADD_REGISTER(
        TYPE_INT,
        ELEMENTINDEX)            //  = 12, /* When multi-element texture is assigned to sampler, this indicates which element index to use.  Default = 0.  */
...
#define USING_LIST "DeviceCaps"
            ADD_REGISTER(TYPE_INT, DeviceType) ADD_REGISTER(TYPE_INT, AdapterOrdinal) ADD_REGISTER(TYPE_INT, Caps) /* Caps from DX7 Draw */
    ADD_REGISTER(TYPE_INT, Caps2) ADD_REGISTER(TYPE_INT, Caps3) ADD_REGISTER(TYPE_INT, PresentationIntervals)
        ADD_REGISTER(TYPE_INT, CursorCaps) /* Cursor Caps */
    ADD_REGISTER(TYPE_INT, DevCaps)        /* 3D Device Caps */
    ADD_REGISTER(TYPE_INT, PrimitiveMiscCaps) ADD_REGISTER(TYPE_INT, RasterCaps) ADD_REGISTER(TYPE_INT, ZCmpCaps) ADD_REGISTER(TYPE_INT, SrcBlendCaps)
        ADD_REGISTER(TYPE_INT, DestBlendCaps) ADD_REGISTER(TYPE_INT, AlphaCmpCaps) ADD_REGISTER(TYPE_INT, ShadeCaps) ADD_REGISTER(TYPE_INT, TextureCaps)
    ...
    ADD_REGISTER(TYPE_INT, MaxTextureWidth) ADD_REGISTER(TYPE_INT, MaxTextureHeight) ADD_REGISTER(TYPE_INT, MaxVolumeExtent) ADD_REGISTER(
        TYPE_INT, MaxTextureRepeat) ADD_REGISTER(TYPE_INT, MaxTextureAspectRatio) ADD_REGISTER(TYPE_INT, MaxAnisotropy) ADD_REGISTER(TYPE_FLOAT, MaxVertexW)
        ADD_REGISTER(TYPE_FLOAT, GuardBandLeft) ADD_REGISTER(TYPE_FLOAT, GuardBandTop) ADD_REGISTER(TYPE_FLOAT, GuardBandRight) ADD_REGISTER(
            TYPE_FLOAT, GuardBandBottom) ADD_REGISTER(TYPE_FLOAT, ExtentsAdjust) ADD_REGISTER(TYPE_INT, StencilCaps) ADD_REGISTER(TYPE_INT, FVFCaps)
            ADD_REGISTER(TYPE_INT, TextureOpCaps) ADD_REGISTER(TYPE_INT, MaxTextureBlendStages) ADD_REGISTER(TYPE_INT, MaxSimultaneousTextures)
                ADD_REGISTER(TYPE_INT, VertexProcessingCaps) ADD_REGISTER(TYPE_INT, MaxActiveLights) ADD_REGISTER(TYPE_INT, MaxUserClipPlanes)
                    ADD_REGISTER(TYPE_INT, MaxVertexBlendMatrices) ADD_REGISTER(TYPE_INT, MaxVertexBlendMatrixIndex) ADD_REGISTER(TYPE_FLOAT, MaxPointSize)
                        ADD_REGISTER(TYPE_INT, MaxPrimitiveCount)            // max number of primitives per DrawPrimitive call
    ADD_REGISTER(TYPE_INT, MaxVertexIndex) ADD_REGISTER(TYPE_INT, MaxStreams)
        ADD_REGISTER(TYPE_INT, MaxStreamStride)                                                           // max stride for SetStreamSource
    ADD_REGISTER(TYPE_INT, VertexShaderVersion) ADD_REGISTER(TYPE_INT, MaxVertexShaderConst)              // number of vertex shader constant registers
    ADD_REGISTER(TYPE_INT, PixelShaderVersion) ADD_REGISTER(TYPE_FLOAT, PixelShader1xMaxValue)            // max value storable in registers of ps.1.x shaders
    ADD_REGISTER(TYPE_INT, DevCaps2)                                                                      // Here are the DX9 specific ones
    ADD_REGISTER(TYPE_FLOAT, MaxNpatchTessellationLevel) ADD_REGISTER(TYPE_INT, Reserved5)
    ...
    ADD_REGISTER(TYPE_INT, VS20Caps.Caps) ADD_REGISTER(TYPE_INT, VS20Caps.DynamicFlowControlDepth) ADD_REGISTER(TYPE_INT, VS20Caps.NumTemps) ADD_REGISTER(
        TYPE_INT, VS20Caps.StaticFlowControlDepth) ADD_REGISTER(TYPE_INT, PS20Caps.Caps) ADD_REGISTER(TYPE_INT, PS20Caps.DynamicFlowControlDepth)
        ADD_REGISTER(TYPE_INT, PS20Caps.NumTemps) ADD_REGISTER(TYPE_INT, PS20Caps.StaticFlowControlDepth) ADD_REGISTER(TYPE_INT, PS20Caps.NumInstructionSlots)
    ...
    ADD_REGISTER(TYPE_INT, MaxVertexShader30InstructionSlots) ADD_REGISTER(TYPE_INT, MaxPixelShader30InstructionSlots)
#undef  USING_STRUCT
#undef  USING_LIST
...
#define USING_LIST "VertexDeclState"
    ...
    ADD_REGISTER(TYPE_INT, PositionT) ADD_REGISTER(TYPE_INT, Normal) ADD_REGISTER(TYPE_INT, Color0) ADD_REGISTER(TYPE_INT, Color1)
        ADD_REGISTER(TYPE_INT, TexCoord0) ADD_REGISTER(TYPE_INT, TexCoord1)
#undef  USING_STRUCT
#undef  USING_LIST
...
void CEffectWrapImpl::AddStateMappedParameter(EStateGroup stateGroup, const SStateVar& var)
{
    switch (stateGroup)
    {
        case STATE_GROUP_RENDER:
            renderStateVarList.push_back(var);
            break;
        case STATE_GROUP_STAGE:
            stageStateVarList.push_back(var);
            break;
        case STATE_GROUP_SAMPLER:
            samplerStateVarList.push_back(var);
            break;
        case STATE_GROUP_MATERIAL:
            materialStateVarList.push_back(var);
            break;
        case STATE_GROUP_TRANSFORM:
            transformStateVarList.push_back(var);
            break;
        case STATE_GROUP_TEXTURE:
            textureStateVarList.push_back(var);
            break;
        case STATE_GROUP_LIGHT:
            lightStateVarList.push_back(var);
            break;
        case STATE_GROUP_LIGHT_ENABLE:
            lightEnableStateVarList.push_back(var);
            break;
        case STATE_GROUP_DEVICE_CAPS:
            deviceCapsVarList.push_back(var);
            break;
        case STATE_GROUP_VERTEX_DECL:
            vertexDeclStateVarList.push_back(var);
            break;
    }
}
