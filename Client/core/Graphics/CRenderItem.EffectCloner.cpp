...
public:
    ...
    virtual ID3DXEffect** CreateD3DEffect(const SString& strFilename, const SString& strRootPath, SString& strOutStatus, bool& bOutUsesVertexShader,
                                          bool& bOutUsesDepthBuffer, bool bDebug);
...
//
////////////////////////////////////////////////////////////////
ID3DXEffect** CEffectClonerImpl::CreateD3DEffect(const SString& strFilename, const SString& strRootPath, SString& strOutStatus, bool& bOutUsesVertexShader,
                                                 bool& bOutUsesDepthBuffer, bool bDebug)
{
    ...
    if (!pNewD3DEffect)
    {
        ...
        m_uiCloneFailTotalCount++;
        {
            ...
            SString strMessage("CloneEffect failed %08x age:%d [clnfail:%d clnsuccess:%d] [totfail:%d totsuccess:%d] [effects cur:%d created:%d dest:%d] '%s'",
                               debugInfo.cloneResult, (CTickCount::Now() - debugInfo.createTime).ToInt(), debugInfo.uiCloneFailCount,
                               debugInfo.uiCloneSuccessCount, m_uiCloneFailTotalCount, m_uiCloneSuccessTotalCount,
                               g_pDeviceState->MemoryState.Effect.iCurrentCount, g_pDeviceState->MemoryState.Effect.iCreatedCount,
                               g_pDeviceState->MemoryState.Effect.iDestroyedCount, *strFilename);
        ...
        if (pEffectTemplatePrevDebug)
        {
            ...
            SString                     strMessage("CloneEffect PrevInfo: %08x age:%d [clnfail:%d clnsuccess:%d]", debugInfo.cloneResult,
                               (CTickCount::Now() - debugInfo.createTime).ToInt(), debugInfo.uiCloneFailCount, debugInfo.uiCloneSuccessCount);
