...
CMainMenu::CMainMenu(CGUI* pManager)
{
    ...
    CVector2D ScreenSize = m_pManager->GetResolution(); /* TODO AFTER CEGUI API REWRITE */
    ...
    CGUITexture* texture = pManager->CreateTexture(CVector2D(1, 1));
    texture->LoadFromFile(CalcMTASAPath(PathJoin("MTA", CORE_MTA_FILLER)));

    m_pFiller = pManager->CreateStaticImage({}, CVector2D(ScreenSize.fX, ScreenSize.fY));
    m_pFiller->LoadFromTexture(texture);
    m_pFiller->SetVisible(false);

    // m_pFiller->SendToFront();
    ...
    //    m_pBackground->SetVisible(false);
    //
    m_pCanvas = reinterpret_cast<CGUIScrollpane*>(pManager->CreateScrollpane(CVector2D(m_iXOff, m_iYOff), CVector2D(m_iMenuSizeX, m_iMenuSizeY)));
    ...
    m_pLogo = reinterpret_cast<CGUIStaticImage*>(
        pManager->CreateStaticImage(CVector2D(0.5f * m_iMenuSizeX - logoSize.fX / 2, 0.365f * m_iMenuSizeY - logoSize.fY / 2), logoSize, m_pCanvas));
    m_pLogo->LoadFromFile(CalcMTASAPath(PathJoin("MTA", CORE_MTA_LOGO)));
    ...
    CVector2D versionPos = CVector2D(m_iMenuSizeX * 0.845f, m_iMenuSizeY * 0.528f);
    CVector2D versionSize = CVector2D((32 / NATIVE_RES_X) * m_iMenuSizeX, (32 / NATIVE_RES_Y) * m_iMenuSizeY);

    m_pVersion = reinterpret_cast<CGUIStaticImage*>(pManager->CreateStaticImage(versionPos, versionSize, m_pCanvas));
    m_pVersion->LoadFromFile(CalcMTASAPath(PathJoin("MTA", CORE_MTA_VERSION)));
    ...
    float fFirstItemSize = m_menuItems.front()->image->GetSize().fY;
    float fSecondItemSize = m_menuItems[1]->image->GetSize().fY;
    ...
    m_pMenuArea->LoadFromFile(CalcMTASAPath(PathJoin("MTA", CORE_MTA_FILLER)));
    ...
    m_pLatestNews->LoadFromFile(CalcMTASAPath(PathJoin("MTA", CORE_MTA_LATEST_NEWS)));
    ...
    for (uint i = 0; i < CORE_MTA_NEWS_ITEMS; i++)
    {
        ...
        CGUILabel* pItemShadow = reinterpret_cast<CGUILabel*>(m_pManager->CreateLabel({}, {}, m_pCanvas, false));
        CGUILabel* pItem = reinterpret_cast<CGUILabel*>(m_pManager->CreateLabel({}, {}, m_pCanvas, false));
    ...
    m_pLanguageSelector = new CLanguageSelector(m_pCanvas); /* TODO AFTER CEGUI API REWRITE */
...
{ /* TODO AFTER CEGUI API REWRITE */
    ...
    float fFirstItemSize = m_menuItems.front()->image->GetSize().fY;
    int   iMoveY = iPosY - m_menuItems.front()->image->GetPosition().fY - fFirstItemSize * 0.5f;

    std::deque<sMenuItem*>::iterator it = m_menuItems.begin();
    for (it; it != m_menuItems.end(); it++)
    {
        CVector2D vOrigPos = (*it)->image->GetPosition();
    ...
    if (m_iMoveStartPos)
    {
        ...
        if (fMoveTime == 1)
        {
            ...
            else
            {
                ...
                float fTopItemSize = m_pDisconnect->image->GetSize().fY;
                float fTopItemCentre = m_pDisconnect->image->GetPosition().fY + fTopItemSize * 0.5f;
...
void CMainMenu::SetVisible(bool bVisible, bool bOverlay, bool bFrameDelay)
{
    ...
    if (!bVisible)
    {
        ...
        SetMenuUnhovered();
        m_ServerBrowser.SetVisible(false);
        // m_Settings.SetVisible(false); /* TODO AFTER CEGUI API REWRITE */
        m_Credits.SetVisible(false);
        m_pNewsBrowser->SetVisible(false);
...
void CMainMenu::SetIsIngame(bool bIsIngame)
{
    ...
    if (m_bIsIngame != bIsIngame)
    {
        ...
        else
        {
            ...
            float fTopItemSize = m_menuItems.front()->image->GetSize().fY;
            float fTopItemCentre = m_menuItems.front()->image->GetPosition().fY + fTopItemSize * 0.5f;
...
void CMainMenu::SetNewsHeadline(int iIndex, const SString& strHeadline, const SString& strDate, bool bIsNew)
{
    ...
    if (pItem->GetSize().fX < pItem->GetTextExtent())
    {
        ...
        for (char i = 0; i < CORE_MTA_NEWS_ITEMS; i++)
        {
            ...
            if (strcmp(szFontName, "default-small") && (m_pNewsItemLabels[i]->GetSize().fX < m_pNewsItemLabels[i]->GetTextExtent()))
            {
