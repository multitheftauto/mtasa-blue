...
bool CQuickConnect::OnConnectButtonClick(CGUIElement* pElement)
{
    ...
    {
        CCore::GetSingleton().ShowMessageBox(_("Error") + _E("CC63"), _("Invalid nickname! Please go to Settings and set a new one!"),
...
void CQuickConnect::LoadData(void)
{
    ...
    CVARS_GET("qc_host", val);
    m_pEditHost->SetText(val.c_str());

    CVARS_GET("qc_port", uval);
    m_pEditPort->SetText(SString("%u", uval));

    CVARS_GET("qc_password", val);
    m_pEditPass->SetText(val.c_str());
}
