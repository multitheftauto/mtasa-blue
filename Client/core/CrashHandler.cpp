...
DWORD BUGSUTIL_DLLINTERFACE __stdcall BSUGetModuleBaseName(HANDLE hProcess, HMODULE hModule, LPTSTR lpBaseName, DWORD nSize);

DWORD BUGSUTIL_DLLINTERFACE __stdcall BSUSymInitialize(DWORD dwPID, HANDLE hProcess, PSTR UserSearchPath, BOOL fInvadeProcess);
...
LONG __stdcall CrashHandlerExceptionFilter(EXCEPTION_POINTERS* pExPtrs)
{
    ...
    __try
    {
        if (NULL != g_pfnCallBack)
        {
            ...
            {
                HINSTANCE hBaseAddr = (HINSTANCE)SymGetModuleBase((HANDLE)GetCurrentProcessId(), (DWORD)pExPtrs->ExceptionRecord->ExceptionAddress);
                if (NULL != hBaseAddr)
                {
...
LPCTSTR __stdcall GetFaultReason(EXCEPTION_POINTERS* pExPtrs)
{
    ...
    __try
    {
        ...
        {
            iCurr += (FormatMessage(FORMAT_MESSAGE_IGNORE_INSERTS | FORMAT_MESSAGE_FROM_HMODULE, GetModuleHandleA(_T("NTDLL.DLL")),
                                    pExPtrs->ExceptionRecord->ExceptionCode, 0, g_szBuff + iCurr, BUFF_SIZE, 0) *
                      sizeof(TCHAR));
        }
...
BOOL __stdcall CH_ReadProcessMemory(HANDLE, LPCVOID lpBaseAddress, LPVOID lpBuffer, DWORD nSize, LPDWORD lpNumberOfBytesRead)
{
...
#define CH_MACHINE IMAGE_FILE_MACHINE_I386
        ...
        BOOL bSWRet = StackWalk(CH_MACHINE, (HANDLE)GetCurrentProcessId(), GetCurrentThread(), &g_stFrame, pExPtrs->ContextRecord,
        ...
        */
        ...
        if (GSTSO_MODULE == (dwOpts & GSTSO_MODULE))
        {
            ...
            ASSERT(iCurr < (BUFF_SIZE - MAX_PATH));
            iCurr += BSUGetModuleBaseName(GetCurrentProcess(), (HINSTANCE)dwModBase, g_szBuff + iCurr, BUFF_SIZE - iCurr);
        }
...
LPCTSTR __stdcall GetRegisterString(EXCEPTION_POINTERS* pExPtrs)
{
    ...
    wsprintf(g_szBuff,
             _T("EAX=%08X  EBX=%08X  ECX=%08X  EDX=%08X  ESI=%08X\n"
                ...
                "FS=%04X  GS=%04X"),
             pExPtrs->ContextRecord->Eax, pExPtrs->ContextRecord->Ebx, pExPtrs->ContextRecord->Ecx, pExPtrs->ContextRecord->Edx, pExPtrs->ContextRecord->Esi,
             pExPtrs->ContextRecord->Edi, pExPtrs->ContextRecord->Ebp, pExPtrs->ContextRecord->Esp, pExPtrs->ContextRecord->Eip, pExPtrs->ContextRecord->EFlags,
             pExPtrs->ContextRecord->SegCs, pExPtrs->ContextRecord->SegDs, pExPtrs->ContextRecord->SegSs, pExPtrs->ContextRecord->SegEs,
...
void InitSymEng(void)
{
    if (FALSE == g_bSymEngInit)
    {
        ...
        VERIFY(BSUSymInitialize((DWORD)hPID, hPID, NULL, TRUE));
        g_bSymEngInit = TRUE;
    }
}
...
DWORD BUGSUTIL_DLLINTERFACE __stdcall BSUSymInitialize(DWORD dwPID, HANDLE hProcess, PSTR UserSearchPath, BOOL fInvadeProcess)
{
    ...
    else
    {
        ...
        for (UINT uiCurr = 0; uiCurr < dwCount; uiCurr++)
        {
            ...
            if (FALSE == GetModuleFileNameA(paMods[uiCurr], szModName, sizeof(szModName)))
            {
...
BOOL BUGSUTIL_DLLINTERFACE __stdcall GetLoadedModules(DWORD dwPID, UINT uiCount, HMODULE* paModArray, LPDWORD pdwRealCount)
{
    ...
    if ((TRUE == IsBadWritePtr(pdwRealCount, sizeof(UINT))) || ((uiCount > 0) && (TRUE == IsBadWritePtr(paModArray, uiCount * sizeof(HMODULE)))))
    {
...
static BOOL InitTOOLHELP32(void)
{
    ...
    if (NULL != hKernel)
    {
        ...
        bRet =
            g_pModule32First && g_pModule32Next && g_pProcess32First && g_pProcess32Next && g_pThread32First && g_pThread32Next && g_pCreateToolhelp32Snapshot;
    }
    else
    {
