...
bool CGUI_Impl::ConvertToModernSkin(const char* skinName)
{
    if (!m_pXML)
    {
    ...
    if (!m_pGraphics)
    {
        AddReportLog(1337, "ConvertToModernSkin: Graphics interface not initialized");
        return false;
    }

    // Duplicate all the CGUI files in the skin folder to a temporary folder
    SString skinPath = CalcMTASAPath(PathJoin("skins", skinName));

    if (!DirectoryExists(skinPath))
    {
        AddReportLog(1337, "ConvertToModernSkin: Skin folder not found");
        return false;
    }

    SString modernPath = PathJoin("MTA", "cgui", "modern");
    SString tempPath = CalcMTASAPath(PathJoin(modernPath, "autogenerated"));
    SString modernVersionPath = CalcMTASAPath(PathJoin(modernPath, ".version"));
    SString tempVersionPath = PathJoin(tempPath, ".version");
    ...
    else
    {
        ...
        std::ifstream modernVersionFile(modernVersionPath);
        std::string   modernVersion;

        if (modernVersionFile.is_open())
        {
            std::getline(modernVersionFile, modernVersion);
            modernVersionFile.close();
        }
        
        std::ifstream tempVersionFile(tempVersionPath);
        std::string   tempVersion;

        if (tempVersionFile.is_open())
        {
            std::getline(tempVersionFile, tempVersion);
            tempVersionFile.close();
        }

        if (!modernVersion.empty() && !tempVersion.empty() && modernVersion == tempVersion)
        {
            // Verify if the skin has changed, or if files already up to date
            SString hashesPath = PathJoin(tempPath, ".integrity");

            if (FileExists(hashesPath))
            {
                std::ifstream hashesFile(hashesPath);

                if (hashesFile.good())
                {
                    std::string checkSkinName;
                    std::getline(hashesFile, checkSkinName);

                    if (checkSkinName == skinName)
                    {
                        SString lookNFeelXmlPath = PathJoin(skinPath, "CGUI.lnf.xml");
                        SString imagesetXmlPath = PathJoin(skinPath, "CGUI.is.xml");
                        SString imagesetPngPath = PathJoin(skinPath, "CGUI.png");
                        SString schemeXmlPath = PathJoin(skinPath, "CGUI.xml");

                        SString lookNFeelXmlHash = CMD5Hasher::CalculateHexString(lookNFeelXmlPath);
                        SString imagesetXmlHash = CMD5Hasher::CalculateHexString(imagesetXmlPath);
                        SString imagesetPngHash = CMD5Hasher::CalculateHexString(imagesetPngPath);
                        SString schemeXmlHash = CMD5Hasher::CalculateHexString(schemeXmlPath);

                        std::string hashes;
                        std::getline(hashesFile, hashes);
    ...
    // Copy all files from requested skin to 'temporary' folder
    SString skinLookNFeelXmlPath = PathJoin(skinPath, "CGUI.lnf.xml");
    SString skinImagesetXmlPath = PathJoin(skinPath, "CGUI.is.xml");
    SString skinImagesetPngPath = PathJoin(skinPath, "CGUI.png");
    SString skinSchemeXmlPath = PathJoin(skinPath, "CGUI.xml");

    if (!FileExists(skinLookNFeelXmlPath) || !FileExists(skinImagesetXmlPath) || !FileExists(skinImagesetPngPath) || !FileExists(skinSchemeXmlPath))
    {
        AddReportLog(1337, "ConvertToModernSkin: Skin files not found");
        return false;
    }

    SString tempLookNFeelXmlPath = PathJoin(tempPath, "CGUI.lnf.xml");
    SString tempImagesetXmlPath = PathJoin(tempPath, "CGUI.is.xml");
    SString tempImagesetPngPath = PathJoin(tempPath, "CGUI.png");
    SString tempSchemeXmlPath = PathJoin(tempPath, "CGUI.xml");

    CopyFile(skinLookNFeelXmlPath, tempLookNFeelXmlPath, 0);
    CopyFile(skinImagesetXmlPath, tempImagesetXmlPath, 0);
    CopyFile(skinImagesetPngPath, tempImagesetPngPath, 0);
    CopyFile(skinSchemeXmlPath, tempSchemeXmlPath, 0);

    if (!FileExists(tempLookNFeelXmlPath) || !FileExists(tempImagesetXmlPath) || !FileExists(tempImagesetPngPath) || !FileExists(tempSchemeXmlPath))
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to copy skin files");
        return false;
    }

    // Store the hashes of each file under .skinname
    SString       integrityFilePath = PathJoin(tempPath, ".integrity");
    std::ofstream integrityFileStream(integrityFilePath, std::ofstream::out | std::ofstream::trunc);

    if (!integrityFileStream.is_open())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to open integrity file stream");
        return false;
    }

    SString lookNFeelXmlHash = CMD5Hasher::CalculateHexString(skinLookNFeelXmlPath);
    SString imagesetXmlHash = CMD5Hasher::CalculateHexString(skinImagesetXmlPath);
    SString imagesetPngHash = CMD5Hasher::CalculateHexString(skinImagesetPngPath);
    SString schemeXmlHash = CMD5Hasher::CalculateHexString(skinSchemeXmlPath);

    integrityFileStream << skinName << std::endl;
    integrityFileStream << lookNFeelXmlHash << imagesetXmlHash << imagesetPngHash << schemeXmlHash;
    integrityFileStream.close();

    // Open the modern XML templates (using TinyXML)
    SString modernTemplatesPath = CalcMTASAPath(PathJoin(modernPath, "templates"));
    SString modernLookNFeelPath = PathJoin(modernTemplatesPath, "looknfeel.xml");
    SString modernSchemePath = PathJoin(modernTemplatesPath, "scheme.xml");
    SString modernImagesetXmlPath = PathJoin(modernTemplatesPath, "imageset.xml");
    SString modernImagesetPngPath = PathJoin(modernTemplatesPath, "imageset.png");

    if (!FileExists(modernLookNFeelPath) || !FileExists(modernSchemePath) || !FileExists(modernImagesetXmlPath) || !FileExists(modernImagesetPngPath))
    {
        AddReportLog(1337, "ConvertToModernSkin: Modern skin templates not found");
        return false;
    }

    // Load the modern LookNFeel, read-only
    CXMLFile* modernLookNFeelFile = m_pXML->CreateXML(modernLookNFeelPath, false, true);

    if (!modernLookNFeelFile || !modernLookNFeelFile->Parse())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to open/parse modern LookNFeel");
        return false;
    }

    // Load the requested skin's LookNFeel, read-write
    CXMLFile* skinLookNFeelFile = m_pXML->CreateXML(tempLookNFeelXmlPath);

    if (!skinLookNFeelFile || !skinLookNFeelFile->Parse())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to open/parse skin LookNFeel");
        return false;
    }

    // Copy the modern LookNFeel into the skin LookNFeel
    CXMLNode* modernRootNode = modernLookNFeelFile->GetRootNode();
    modernRootNode->CopyChildrenInto(skinLookNFeelFile->GetRootNode(), true, false);

    // Save the skin LookNFeel
    if (!skinLookNFeelFile->Write())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to write skin LookNFeel");
        return false;
    }

    delete modernLookNFeelFile;
    delete skinLookNFeelFile;

    // Load the modern scheme, read-only
    CXMLFile* modernSchemeFile = m_pXML->CreateXML(modernSchemePath, false, true);

    if (!modernSchemeFile || !modernSchemeFile->Parse())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to open/parse modern scheme");
        return false;
    }

    // Load the requested skin's scheme, read-write
    CXMLFile* skinSchemeFile = m_pXML->CreateXML(tempSchemeXmlPath);

    if (!skinSchemeFile || !skinSchemeFile->Parse())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to open/parse skin scheme");
        return false;
    }

    // Copy the modern scheme into the skin scheme
    modernRootNode = modernSchemeFile->GetRootNode();
    modernRootNode->CopyChildrenInto(skinSchemeFile->GetRootNode(), true, false);

    // Save the skin scheme
    if (!skinSchemeFile->Write())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to write skin scheme");
        return false;
    }

    delete modernSchemeFile;
    delete skinSchemeFile;

    // Load the imagesets as CTextureItem
    auto renderManager = m_pGraphics->GetRenderItemManager();
    auto modernImagesetTexture = renderManager->CreateTexture(modernImagesetPngPath);
    auto skinImagesetTexture = renderManager->CreateTexture(skinImagesetPngPath);

    if (!modernImagesetTexture || !skinImagesetTexture)
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to create imageset textures");
        return false;
    }

    // Create a render target to hold both imagesets
    int width = modernImagesetTexture->m_uiSizeX + skinImagesetTexture->m_uiSizeX;
    auto renderTarget =
    ...
    // Start draw batch
    renderManager->SetRenderTarget(renderTarget, true);

    // Draw the imagesets side-by-side
    m_pGraphics->DrawTexture(skinImagesetTexture, 0, 0, 1, 1, 0, 0, 0, 0xFFFFFFFF, 0, 0, 1, 1, true);
    m_pGraphics->DrawTexture(modernImagesetTexture, skinImagesetTexture->m_uiSizeX, 0, 1, 1, 0, 0, 0, 0xFFFFFFFF, 0, 0, 1, 1, true);
    ...
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to save imageset texture");
        return false;
    }
    ...
    // Load the modern imageset, read-only
    CXMLFile* modernImagesetFile = m_pXML->CreateXML(modernImagesetXmlPath, false, true);

    if (!modernImagesetFile || !modernImagesetFile->Parse())
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to open/parse modern imageset");
        return false;
    }

    // Adjust each Image in the modern imageset to have the correct offset (XPos, YPos)
    modernRootNode = modernImagesetFile->GetRootNode();

    for (uint i = 0; i < modernRootNode->GetSubNodeCount(); i++)
    {
        CXMLNode* node = modernRootNode->GetSubNode(i);
        CXMLAttributes& attributes = node->GetAttributes();

        auto xPos = attributes.Find("XPos");

        if (xPos)
        {
            try
            {
                auto x = std::stoi(xPos->GetValue());
                xPos->SetValue((uint)x + skinImagesetTexture->m_uiSizeX);
            }
            catch (std::invalid_argument)
            {
                AddReportLog(1337, "ConvertToModernSkin: Invalid XPos value in imageset");
                return false;
            }
        }
    }
    ...
    CopyFile(modernVersionPath, tempVersionPath, 0);

    if (!FileExists(tempVersionPath))
    {
        AddReportLog(1337, "ConvertToModernSkin: Failed to copy modern version file");
        return false;
    }
