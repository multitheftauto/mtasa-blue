...
{
    auto pVehicleEntity = pGameInterface->GetPools()->GetVehicle((DWORD*)vehicle);

    if (pVehicleEntity->pEntity && m_pBreakTowLinkHandler)
    {
        return m_pBreakTowLinkHandler(pVehicleEntity->pEntity);
    }
    return true;
}
