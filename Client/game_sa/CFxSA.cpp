...
void CFxSA::AddParticle(eFxParticleSystems eFxParticle, CVector& vecPosition, CVector& vecDirection, float fR, float fG, float fB, float fA, bool bRandomizeColors, int iCount, float fBrightness, float fSize, bool bRandomizeSizes, float fLife)
{
    ...
    for (int i = 0; i < iCount; i++)
    {
        ...
        {
            // 0x49EECB
            fxPrt.m_color.red = (rand() % 10000) * 0.0001 * fR + 0.13;
            fxPrt.m_color.green = (rand() % 10000) * 0.0001 * fG + 0.12;
            fxPrt.m_color.blue = (rand() % 10000) * 0.0001 * fB + 0.04;
        }

        if (bRandomizeSizes)
            // 0x49EF21 - Calculate random size for each particle
            fxPrt.m_fSize = (rand() % 10000) * 0.0001 * fSize + 0.3;
        ...
        newDirection.fX = (rand() % 10000) * 0.0001 * 4.0 - 2.0 + newDirection.fX;
        newDirection.fY = (rand() % 10000) * 0.0001 * 4.0 - 2.0 + newDirection.fY;
        newDirection.fZ = (rand() % 10000) * 0.0001 * 4.0 - 2.0 + newDirection.fZ;

        // Call FxSystem_c::AddParticle
        ((int(__thiscall*)(FxSystem_c*, CVector*, CVector*, float, FxPrtMult_c*, float, float, float, int))FUNC_FXSystem_c_AddParticle)(fxParticleSystem, &vecPosition, &newDirection, 0, &fxPrt, -1.0f, fBrightness, 0, 0);
    }
}
