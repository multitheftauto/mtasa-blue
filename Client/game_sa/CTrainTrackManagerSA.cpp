...
std::map< unsigned int, SRailNodeSA > CTrainTrackManagerSA::m_OriginalTrainTrackData[4];
...
bool CTrainTrackManagerSA::GetOriginalRailNode ( unsigned char ucTrack, unsigned short usNode, SRailNodeSA * pRailNode )
{
    if ( ucTrack < NUM_RAILTRACKS )
    {
        if ( m_OriginalTrainTrackData[ucTrack].find ( usNode ) != m_OriginalTrainTrackData[ucTrack].end ( ) )
        {
            SRailNodeSA * pOriginalRailNode = m_pTrainTracks[ucTrack]->GetRailNode ( usNode );
            pRailNode->padding = pOriginalRailNode->padding;
            pRailNode->sX = pOriginalRailNode->sX;
            pRailNode->sY = pOriginalRailNode->sY;
            pRailNode->sZ = pOriginalRailNode->sZ;
            pRailNode->sRailDistance = pOriginalRailNode->sRailDistance;
            return true;
        }
    }
    return false;
...
CTrainTrackSA * CTrainTrackManagerSA::CreateTrainTrack ( unsigned int uiNodes, unsigned char ucTrackID, bool bLinkedLastNode )
{
    ...
    {
        CTrainTrackSA * pTrainTrack = new CTrainTrackSA ( ucTrackID, false, this );
...
bool CTrainTrackManagerSA::Reallocate ( CTrainTrackSA * pTrainTrack, unsigned int uiNodes )
{
    // Get our track ID
    DWORD dwTrackID = pTrainTrack->GetTrackID ( );
    // Allocate memory for our nodes
    void * pMemory = malloc ( sizeof ( SRailNodeSA ) * uiNodes );
    
    // loop through nodes
    for ( unsigned int i = 0; i < uiNodes; i++ )
    {
        // grab our node at index i
        SRailNodeSA * pNode = (SRailNodeSA *)pMemory + i;
    ...
    // Set our data
    m_pRailNodePointers[dwTrackID]     = pMemory;
    ...
    m_pTrainTracks[dwTrackID]->Recalculate ( );
    return true;
}
...
void * CTrainTrackManagerSA::AllocateDefault ( void )
{
    ...
    // Allocate memory for our nodes
    void * pMemory = malloc ( sizeof ( SRailNodeSA ) * 10 );
