...
CObjectSA::CObjectSA(DWORD dwModel, bool bBreakingDisabled)
{
    ...
    DWORD dwObjectPtr = 0;
    // clang-format off
    using func_t = decltype(dwObjectPtr) (__cdecl*)(decltype(dwModel), decltype(1));
dwObjectPtr =     reinterpret_cast<func_t>(CObjectCreate)(dwModel, 1);
    // clang-format on
...
void CObjectSA::Break()
{
    ...
    // clang-format off
    using func_t = void (__thiscall*)(decltype(dwThis), decltype(fHitVelocity), decltype(0), decltype(0x0B73710), decltype(0), decltype(0x32));
    reinterpret_cast<func_t>(dwFunc)(dwThis, fHitVelocity, 0, 0x0B73710, 0, 0x32);
    // clang-format on
    ...
    if (IsGlass())
    {
        ...
        // clang-format off
        using func_t = void (__cdecl*)(decltype(dwThis), decltype(fHitVelocity), decltype(0), decltype(0), decltype(0), decltype(fX), decltype(fY), decltype(fZ), decltype(0));
    reinterpret_cast<func_t>(dwFunc)(dwThis, fHitVelocity, 0, 0, 0, fX, fY, fZ, 0);
        // clang-format on
    }
}
...
bool CObjectSA::IsGlass()
{
    ...
    // clang-format off
    using func_t = decltype(bResult) (__cdecl*)(decltype(dwThis));
bResult =     reinterpret_cast<func_t>(dwFunc)(dwThis);
    // clang-format on
    return bResult;
}
