...
CWaterManagerSA::~CWaterManagerSA()
{
	//Platinum Edit 17.4 - We depend on FLA for water anyway...
	//UndoChanges();
...
{
	if (fX < -12000.0f || fX > 12000.0f || fY < -12000.0f || fY > 12000.0f)
		return NULL;
	...
	int zoneID = 12 * ((int)(fX + 12000.0f) / 500) + (int)(fY + 12000.0f) / 500;
...
{
	out.clear();
	float fColumnLeft = -12000.0f;
	for (int column = 0; column < 12; column++)
	{
		float fRowBottom = -12000.0f;
...
{
	if (vecPosition.fX < -12000.0f || vecPosition.fX > 12000.0f || vecPosition.fY < -12000.0f || vecPosition.fY > 12000.0f)
		return NULL;
...
CWaterPoly* CWaterManagerSA::CreateQuad(const CVector& vecBL, const CVector& vecBR, const CVector& vecTL, const CVector& vecTR, bool bShallow)
{
	...
	if (vecTL.fX >= vecTR.fX || vecBL.fX >= vecBR.fX ||
		...
		vecTL.fX < -12000.0f || vecTL.fX > 12000.0f || vecTL.fY < -12000.0f || vecTL.fY > 12000.0f ||
		vecTR.fX < -12000.0f || vecTR.fX > 12000.0f || vecTR.fY < -12000.0f || vecTR.fY > 12000.0f ||
		vecBL.fX < -12000.0f || vecBL.fX > 12000.0f || vecBL.fY < -12000.0f || vecBL.fY > 12000.0f ||
		vecBR.fX < -12000.0f || vecBR.fX > 12000.0f || vecBR.fY < -12000.0f || vecBR.fY > 12000.0f)
		return NULL;
...
CWaterPoly* CWaterManagerSA::CreateTriangle(const CVector& vec1, const CVector& vec2, const CVector& vec3, bool bShallow)
{
	...
	if (vec1.fX >= vec2.fX || vec1.fY == vec3.fY || vec2.fY == vec3.fY ||
		...
		vec1.fX < -12000.0f || vec1.fX > 12000.0f || vec1.fY < -12000.0f || vec1.fY > 12000.0f ||
		vec2.fX < -12000.0f || vec2.fX > 12000.0f || vec2.fY < -12000.0f || vec2.fY > 12000.0f ||
		vec3.fX < -12000.0f || vec3.fX > 12000.0f || vec3.fY < -12000.0f || vec3.fY > 12000.0f)
		return NULL;
...
void CWaterManagerSA::Reset()
{
	...
	//Platinum Edit 17.3 - We depend on FLA for water anyway...
	UndoChanges();
