...
enum class eModelSpecialType : std::uint8_t
{
    NONE = 0,
    TREE = 1,
    PALM = 2,
...
public:
    ...
    std::uint32_t  ulHashKey;            // +4   Generated by CKeyGen::GetUppercaseKey(char const *) called by CBaseModelInfo::SetModelName(char const *)
    std::uint16_t usNumberOfRefs : 16;                 // +8
    std::uint16_t usTextureDictionary : 16;            // +10
    std::uint8_t  ucAlpha : 8;                         // +12

    std::uint8_t  ucNumOf2DEffects : 8;            // +13
    std::uint16_t usUnknown : 16;                  // +14     Something with 2d effects

    std::uint16_t usDynamicIndex : 16;            // +16
    ...
    union
    {
        struct
        {
            std::uint8_t     bHasBeenPreRendered : 1;            // we use this because we need to apply changes only once
            std::uint8_t     bAlphaTransparency : 1;             // bDrawLast
            std::uint8_t     bAdditiveRender : 1;
            std::uint8_t     bDontWriteZBuffer : 1;
            std::uint8_t     bDontCastShadowsOn : 1;
            std::uint8_t     bDoWeOwnTheColModel : 1;
            std::uint8_t     bIsBackfaceCulled : 1;
            std::uint8_t     bIsColLoaded : 1;
            std::uint8_t     bIsRoad : 1;
            std::uint8_t     bHasComplexHierarchy : 1;
            std::uint8_t     bDontCollideWithFlyer : 1;
            eModelSpecialType eSpecialModelType : 4;
            std::uint8_t     bWetRoadReflection : 1;            // Used for tags
        };
...
public:
    ...
    char                                treeColors[8];
    char                                fourColors[8];
    std::uint8_t                       ucNumOfColorVariations;
    std::uint8_t                       ucLastColorVariation;
    std::uint8_t                       ucPrimColor;
    std::uint8_t                       ucSecColor;
    std::uint8_t                       ucTertColor;
    std::uint8_t                       ucQuatColor;
    char                                upgrades[36];
    char                                anRemapTXDs[8];
    char                                pad5[2];
    char                                pAnimBlock[4];
...
protected:
    ...
    static std::map<std::uint16_t, int>                                         ms_RestreamTxdIDMap;
    static std::map<DWORD, float>                                                ms_ModelDefaultLodDistanceMap;
    static std::map<DWORD, std::uint16_t>                                       ms_ModelDefaultFlagsMap;
    ...
    static std::unordered_map<DWORD, std::uint16_t>                             ms_OriginalObjectPropertiesGroups;
    static std::unordered_map<DWORD, std::pair<float, float>>                    ms_VehicleModelDefaultWheelSizes;
    static std::map<std::uint16_t, int>                                         ms_DefaultTxdIDMap;
...
public:
    ...
    bool           IsLoaded();
    bool           DoIsLoaded();
    std::uint16_t GetFlags();
    std::uint16_t GetOriginalFlags();
    void           SetIdeFlags(std::uint32_t uiFlags);
    void           SetIdeFlag(eModelIdeFlag eIdeFlag, bool bState);
    bool           GetIdeFlag(eModelIdeFlag eIdeFlag);
    void           SetFlags(std::uint16_t usFlags);
    static void    StaticResetFlags();
    CBoundingBox*  GetBoundingBox();
    bool           IsValid();
    bool           IsAllocatedInArchive();
    float          GetDistanceFromCentreOfMassToBaseOfModel();
    std::uint16_t GetTextureDictionaryID();
    void           SetTextureDictionaryID(std::uint16_t usID);
    ...
    std::uint32_t GetParentID() { return m_dwParentID; };
    ...
    void           SetObjectPropertiesGroup(std::uint16_t usObjectGroup);
    std::uint16_t GetObjectPropertiesGroup();
