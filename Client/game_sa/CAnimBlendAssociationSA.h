 ...
 *  FILE:        Client/game_sa/CAnimBlendAssociationSA.h
...
public:
    union
    {
        struct
        {
            uchar m_bf1 : 1;                                             // doesn't seem to be used
            uchar m_IsIFrameOrientationToAffectedByNodes : 1;            // m_IFrame orientation will be affected
            uchar m_IsIFrameTranslationToAffectedByNodes : 1;            // m_IFrame translation will be affected
            uchar m_bIsInitialized : 1;
            uchar m_bUpdateSkinnedWith3dVelocityExtraction : 1;
            uchar m_bCheckBlendNodeClumpKeyFrames : 1;            // key frames of CAninBlendNode bones will be checked
            uchar m_bIsCompressed : 1;
            uchar m_bUpdatingFrame : 1;            // doesn't seem to be used
        };
        uchar m_nFlags;
    };
    char                     pad[3];
...
public:
    ...
    AnimBlendFrameData* GetFrameDataByNodeId(uint uiID);
};
...
public:
    ...
    union
    {
        struct
        {
            ushort m_bPlaying : 1;                    // Anim will stop playing if flag is not set
            ushort m_bLooped : 1;                     // Anim will always restart when it completes
            ushort m_bFreezeLastFrame : 1;            // Anim will freeze on last frame
            ...
            ushort m_bLockLastFrame : 1;

            ushort m_bPartial : 1;                   // Partial anims run along other anims
            ushort m_bEnableMovement : 1;            // blends all playing anims together if set

            // Camera will not move with ped, and ped will go back to the initial position when anim is over
            ushort m_bLockLastX : 1;
            ushort m_bLockLastY : 1;            // only applies if m_bLockLastX is set

            ushort m_bf9 : 1;             // doesn't seem to be used
            ushort m_bf10 : 1;            // doesn't seem to be used

            // If set to TRUE, then result:
            // Before = https://i.imgur.com/c8T7xNK.png | AFTER = https://i.imgur.com/4gqlA4n.png
            ushort m_bAddAnimBlendToTotalBlend : 1;

            ushort m_bf12 : 1;                          // doesn't seem to be used
            ushort m_bSecondaryTaskAnim : 1;            // doesn't seem to be used
            ...
            ushort m_bFreezeTranslation : 1;
            ushort m_bBlockReferenced : 1;            // anim block can't be unloaded if it's referenced by an anim.
            ...
            ushort m_bIndestructible : 1;
        };
        ushort m_nFlags;
    };
    uint   uiCallbackType;            // 48:  1 means finished and 2 means delete
