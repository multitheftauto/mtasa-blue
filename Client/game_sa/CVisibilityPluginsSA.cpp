...
void CVisibilityPluginsSA::SetClumpAlpha(RpClump* pClump, int iAlpha)
{
    ...
    // clang-format off
    using func_t = void (__cdecl*)(decltype(pClump), decltype(iAlpha));
    reinterpret_cast<func_t>(dwFunc)(pClump, iAlpha);
    // clang-format on
}
...
int CVisibilityPluginsSA::GetAtomicId(RwObject* pAtomic)
{
    ...
    int   iResult = 0;
    // clang-format off
    using func_t = decltype(iResult) (__cdecl*)(decltype(pAtomic));
iResult =     reinterpret_cast<func_t>(dwFunc)(pAtomic);
    // clang-format on
    return iResult;
}
