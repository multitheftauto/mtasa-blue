...
////////////////////////////////////////////////////////////////////
//
// Modificado el de fileso de sourceo
//

// Code is from NSIS plugin AccessControl.
// This block gets the code with working in VS
...
void pushstring ( const std::string& szText )
{
    stringStack.push_back ( szText );
}

int popstring ( char* szText )
{
    if ( stringStack.size () )
    {
        std::string strText = stringStack.back ();
        stringStack.pop_back ();

        strText = strText.substr ( 0, g_string_size - 4 );
        strncpy ( szText, strText.c_str (), strText.length () + 1 );
        return 0;
    }
    return 1;
}
...
void DumpResult ()
{
    while (  stringStack.size () )
    {
        std::string strText = stringStack.back ();
        stringStack.pop_back ();
        MessageBox( NULL, strText.c_str (), "", MB_ICONEXCLAMATION|MB_OK );
    }
}

// SetOnFile [/NOINHERIT] <filename> <trustee> <permissions>
extern "C" void __declspec(dllexport) SetOnFile(HWND hwndParent, int string_size,
...
{
    pushstring ( strPermissions );
    pushstring ( strTrustee );
    pushstring ( strFilename );
    if ( !bYesInherit )
...
char * ParseSid(const TCHAR * intrustee)
{
    char buffer[256];
    strncpy( buffer, intrustee, 250 );
    TCHAR * trustee = buffer;
