...
*  FILE:        mods/deathmatch/logic/CPlayerClothes.cpp
...
SPlayerClothing* CPlayerClothes::GetClothing ( unsigned char ucType )
{
...
bool CPlayerClothes::IsEmptyClothing ( SPlayerClothing * pClothing, unsigned char ucType )
{
    if ( pClothing )
    {
        if ( ucType <= 3 )
...
char* CPlayerClothes::GetClothingName ( unsigned char ucType )
{
    if ( ucType < PLAYER_CLOTHING_SLOTS )
    {
...
SPlayerClothing* CPlayerClothes::GetClothingGroup ( unsigned char ucType )
{
    if ( ucType < PLAYER_CLOTHING_SLOTS )
    {
        switch ( ucType )
...
bool CPlayerClothes::IsValidClothing ( const char* szTexture, const char* szModel, unsigned char ucType )
{
    if ( ucType < PLAYER_CLOTHING_SLOTS )
    {
        SPlayerClothing* pPlayerClothing = GetClothingGroup ( ucType );
        if ( pPlayerClothing )
        {
            for ( int i = 0 ; pPlayerClothing [ i ].szTexture != NULL ; i++ )
            {
                if ( !stricmp ( pPlayerClothing [ i ].szTexture, szTexture ) &&
                     !stricmp ( pPlayerClothing [ i ].szModel, szModel ) )
                {
                    return true;
                }
            }
