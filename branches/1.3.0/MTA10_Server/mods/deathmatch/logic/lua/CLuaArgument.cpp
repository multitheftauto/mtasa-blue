    ...
    if ( bitStream.Read ( &type ) )
    {
        ...
        switch ( type.data.ucType )
        {
            ...
            case LUA_TSTRING_LONG:
            {
                ...
                if ( uiLength > 65535 && g_pGame->GetConfig()->GetMinClientVersion () < LONG_STRING_MIN_VERSION )
                {
...
bool CLuaArgument::WriteToBitStream ( NetBitStreamInterface& bitStream, std::map < CLuaArguments*, unsigned long > * pKnownTables ) const
{
    ...
    switch ( GetType () )
    {
        ...
        {           
            ...
            if ( sizeTemp > 65535 && bitStream.Version () >= 0x027 && g_pGame->GetConfig()->GetMinClientVersion () >= LONG_STRING_MIN_VERSION )
            {
