...
void CALLBACK DownloadSync ( HSYNC handle, DWORD channel, DWORD data, void* user )
{
    ...
    //g_pCore->GetConsole()->Printf ( "BASS ERROR %d DownloadSync  sound = %d  tags = %s", BASS_ErrorGetCode(), channel, TAGS_Read( channel, "%IFV1(%TRCK,%TRCK. )%IFV2(%ARTI,%ICAP(%ARTI),no artist) - %IFV2(%TITL,%ICAP(%TITL),no title)%IFV1(%ALBM, - %IUPC(%ALBM))%IFV1(%YEAR, %(%YEAR%))" ) );
...
void CClientSound::ThreadCallback ( HSTREAM pSound )
{
    if ( pSound )
    {
        ...
        //g_pCore->GetConsole()->Printf ( "BASS ChannelSetSync  sound = %d  freq = %d  tags = %s", pSound, m_fDefaultFrequency, TAGS_Read( pSound, "%IFV1(%TRCK,%TRCK. )%IFV2(%ARTI,%ICAP(%ARTI),no artist) - %IFV2(%TITL,%ICAP(%TITL),no title)%IFV1(%ALBM, - %IUPC(%ALBM))%IFV1(%YEAR, %(%YEAR%))" ) );

        BASS_ChannelPlay ( pSound, false );

        // Call onClientSoundStart LUA event
        CLuaArguments Arguments;
        Arguments.PushNumber ( GetLength () );
        this->CallEvent ( "onClientSoundStart", Arguments, true );
    }
    else
        g_pCore->GetConsole()->Printf ( "BASS ERROR %d PlayStream  b3D = %s  path = %s", BASS_ErrorGetCode(), m_b3D ? "true" : "false", m_strPath.c_str() );
