...
void CGUILabel_Impl::AutoSize ( const char* Text )
{
    ...
    m_pWindow->setSize ( CEGUI::Absolute, CEGUI::Size ( pFont->getTextExtent ( Text ? Text : GetText() ), pFont->getFontHeight() + 2.0f ) );   // Add hack factor to height to allow for long characters such as 'g' or 'j'
}
