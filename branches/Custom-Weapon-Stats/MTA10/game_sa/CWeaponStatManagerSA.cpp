...
*  FILE:        mods/deathmatch/logic/CWeaponStatManagerSA.h
...
sWeaponInfo CWeaponStatManagerSA::OriginalWeaponData [ WEAPONTYPE_MAX+1 ];
...
CWeaponStatManagerSA::~CWeaponStatManagerSA ( )
{
    for ( std::list < CWeaponStatSA* >::iterator iter = m_OriginalWeaponData.begin (); iter != m_OriginalWeaponData.end ();iter++ )
    {
        if ( (*iter ) )
            delete (*iter);
    }

    for ( std::list < CWeaponStatSA* >::iterator iter = m_WeaponData.begin (); iter != m_WeaponData.end ();iter++ )
    {
        if ( (*iter ) )
            delete (*iter);
    }
...
CWeaponStatSA* CWeaponStatManagerSA::GetWeaponStats ( eWeaponType type, eWeaponSkill skill )
{
    for ( std::list < CWeaponStatSA* >::iterator iter = m_OriginalWeaponData.begin (); iter != m_OriginalWeaponData.end ();iter++ )
    {
...
CWeaponStatSA* CWeaponStatManagerSA::GetOriginalWeaponStats ( eWeaponType type, eWeaponSkill skill )
{
    for ( std::list < CWeaponStatSA* >::iterator iter = m_OriginalWeaponData.begin (); iter != m_OriginalWeaponData.end ();iter++ )
    {
...
bool CWeaponStatManagerSA::LoadDefault ( CWeaponStat* pDest, eWeaponType weaponType )
{
    if ( pDest )
    {
