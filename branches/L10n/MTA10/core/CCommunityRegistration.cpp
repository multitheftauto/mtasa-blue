...
void CCommunityRegistration::DoPulse ( void )
{
    if ( m_ulStartTime > 0 )
    {
        ...
        if ( m_HTTP.GetData ( buffer ) )
        {
            ...
            {
                g_pCore->ShowMessageBox ( _("Success"), _("Successfully registered!"), MB_BUTTON_OK | MB_ICON_INFO );
            ...
            else if ( Result == REGISTRATION_ERROR_ERROR )
            {
                if ( strlen ( &szBuffer[1] ) > 0 )
                    g_pCore->ShowMessageBox ( _("Error"), &szBuffer[1], MB_BUTTON_OK | MB_ICON_ERROR );
                else
                    g_pCore->ShowMessageBox ( _("Error"), "Unexpected error", MB_BUTTON_OK | MB_ICON_ERROR );
...
bool CCommunityRegistration::OnButtonRegisterClick ( CGUIElement* pElement )
{
    if ( m_pEditUsername->GetText().empty() )
        g_pCore->ShowMessageBox ( _("Error"), _("Username missing"), MB_BUTTON_OK | MB_ICON_INFO );
    else if ( m_pEditEmail->GetText().empty() )
        g_pCore->ShowMessageBox ( _("Error"), _("Email missing"), MB_BUTTON_OK | MB_ICON_INFO );
    else if ( m_pEditPassword->GetText().empty() || m_pEditConfirm->GetText().empty() )
        g_pCore->ShowMessageBox ( _("Error"), _("Password missing"), MB_BUTTON_OK | MB_ICON_INFO );
    else if ( m_pEditPassword->GetText() != m_pEditConfirm->GetText() )
        g_pCore->ShowMessageBox ( _("Error"), _("Passwords do not match"), MB_BUTTON_OK | MB_ICON_INFO );
    else if ( m_pEditCode->GetText().empty() )
        g_pCore->ShowMessageBox ( _("Error"), _("Validation code missing"), MB_BUTTON_OK | MB_ICON_INFO );
    else
    {
        if ( m_ulStartTime == 0 )
        {
