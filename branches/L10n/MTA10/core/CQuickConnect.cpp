...
bool CQuickConnect::OnConnectButtonClick ( CGUIElement* pElement )
{
    ...
    if ( strHost.length () == 0 )
    {
        CCore::GetSingleton ().ShowMessageBox ( _("Error"), _("No host specified!"), MB_BUTTON_OK | MB_ICON_INFO );
        return true;
    }
    ...
    if ( strPort.length () == 0 )
    {
        CCore::GetSingleton ().ShowMessageBox ( _("Error"), _("No port specified!"), MB_BUTTON_OK | MB_ICON_INFO );
        return true;
    }
    else if ( iPort == 0 || iPort > 0xFFFF )
    {
        CCore::GetSingleton ().ShowMessageBox ( _("Error"), _("Invalid port specified!"), MB_BUTTON_OK | MB_ICON_INFO );
        return true;
    }
