    ...
    if ( !argStream.HasErrors () )
    {
        ...
        if ( pLuaMain )
        {
            ...
            if ( CResourceManager::ParseResourcePathInput( path, pResource, strPath ) )
            {
                ...
                m_pScriptDebugging->LogCustom ( luaVM, SString ( "Bad file path @ '%s' [%s]", lua_tostring ( luaVM, lua_upvalueindex ( 1 ) ), *strPath ) );
        }
    }
    else
        m_pScriptDebugging->LogCustom ( luaVM, SString ( "Bad argument @ '%s' [%s]", lua_tostring ( luaVM, lua_upvalueindex ( 1 ) ), *argStream.GetErrorMessage () ) );
        ...
        if ( pLuaMain )
        {
            ...
            if ( CResourceManager::ParseResourcePathInput( filename, pResource, strPath ) )
            {
                if ( CStaticFunctionDefinitions::GUIStaticImageLoadImage ( *theElement, strPath ) )
                {
                    ...
                    m_pScriptDebugging->LogCustom ( luaVM, SString ( "Bad news @ '%s' [%s '%s']", lua_tostring ( luaVM, lua_upvalueindex ( 1 ) ), "Problem loading image", *strPath ) );
            }
            else
                m_pScriptDebugging->LogCustom ( luaVM, SString ( "Bad file path @ '%s' [%s]", lua_tostring ( luaVM, lua_upvalueindex ( 1 ) ), *strPath ) );
        }
