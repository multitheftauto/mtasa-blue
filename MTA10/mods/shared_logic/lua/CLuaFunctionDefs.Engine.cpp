...
int CLuaFunctionDefs::EngineSetFramePosition ( lua_State* luaVM )
{
    CScriptArgReader argStream ( luaVM );
    ...
    if ( !argStream.HasErrors() )
    {
        if ( pVehicle )
        { 
            if ( pVehicle->SetComponentPosition ( Component, vecPosition ) )
            {
                lua_pushboolean ( luaVM, true );
                return 1;
            }
...
int CLuaFunctionDefs::EngineGetFramePosition ( lua_State* luaVM )
{
    CScriptArgReader argStream ( luaVM );
    eVehicleComponent Component;
    CClientVehicle * pVehicle = NULL;
    ...
    if ( !argStream.HasErrors() )
    {
        if ( pVehicle )
        {
            if ( pVehicle->GetComponentPosition ( Component, vecPosition ) )
            {
